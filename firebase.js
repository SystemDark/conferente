...
let motoristaEntrada = document.getElementById("motoristaEntrada").value;
let motoristaSaida = document.getElementById("motoristaSaida").value;
...
const q = query(collection(db,"containers"),orderBy("horario","desc"));
...
let horario = c.horario?.toDate().toLocaleString() || "-";
...
<td>${c.empresa || "-"}</td>
<td>${c.localizacao}</td>
<td>${c.motoristaEntrada || "-"}</td>
<td>${c.motoristaSaida || "-"}</td>
<td>${horario}</td>
<td>${c.prazo || "-"}</td>
<td>
<button class="editBtn" onclick="editar('${id}')">Editar</button>
</td>
</tr>
`;
...
let novoLocal = prompt("Nova localização do container:");
if(!novoLocal) return;
